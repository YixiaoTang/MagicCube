...
namespace Com.MyCompany.MyGame
{
    public class BallModule : MonoBehaviour, IComparable
    {
        ...
        {
            if (ballCurrentLevel < GameManager.Instance.MaxLevel)
            {
                ballCurrentLevel++;
                gameObject.GetComponent<Renderer>().material = levelMatrials[ballCurrentLevel - 1];
                ballSizeCurrent = ballSizeInit + 5 * GameManager.Instance.ballSizeStep * (ballCurrentLevel - 1);
                gameObject.transform.localScale += new Vector3(1, 1, 1) * GameManager.Instance.ballSizeStep;
            }
        }
        ...
        private void LevelDown()
        {
            if (ballCurrentLevel > 1)
            {
                ballCurrentLevel--;
                gameObject.GetComponent<Renderer>().material = levelMatrials[ballCurrentLevel - 1];
                ballSizeCurrent = ballSizeInit + GameManager.Instance.ballSizeStep * (ballCurrentLevel - 1);
                gameObject.transform.localScale -= new Vector3(1, 1, 1) * GameManager.Instance.ballSizeStep;
            }
        }
